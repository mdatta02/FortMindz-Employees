...
const ListEmployee = () => {
  ...
  const columns = [
    columnHelper.accessor("image", {
      cell: (info) => <img src={info.getValue()} height="50" width="50" />,
      header: "Image",
    }),
      ...
      cell: (info) => (
        <>
          <button
            ...
            onClick={() => editEmployeeHandler(info.getValue())}
          >
            Edit
          </button>
          <button
            ...
            onClick={() => removeEmployeeHandler(info.getValue())}
          >
            Delete
          </button>
        </>
  ...
  useEffect(() => {
    if (employeeList && employeeList.length) {
      console.log("hhh", totalEmployees);
  ...
  }, [employeeList, totalEmployees]);

  const fetchData = useCallback((currentPagination) => {
    setPagination(currentPagination);
    dispatch(listEmployee(currentPagination));
  }, []);
  ...
  return (
    <>
      <section id="employee-list" className="p-2">
        <ToastContainer />
        {loading && (
